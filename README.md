...
```sh
# 自動的に管理者権限に昇格
./gopier.exe -s ./src -d ./dst --preserve-permissions --auto-elevate
```

#### 3. 権限コピーを無効化
```sh
# 権限コピーを無効にしてコピー
./gopier.exe -s ./src -d ./dst --no-preserve-permissions
```

#### 4. エラーの詳細確認
```sh
# 詳細ログでエラーを確認
./gopier.exe -s ./src -d ./dst --preserve-permissions --verbose
```

#### よくあるエラーと対処法

- **アクセス拒否エラー**: 管理者権限で実行してください
- **特権不足エラー**: セキュリティ特権が必要です
- **所有者設定エラー**: 自動的にアクセス権限（DACL）のみをコピーします
- **詳細不明エラー**: 詳細ログ（`--verbose`）で原因を確認してください

#### 自動フォールバック機能

権限コピーでエラーが発生した場合、以下の順序で処理を試行します：

1. **完全な権限コピー**: 所有者、グループ、DACLをすべてコピー
2. **DACLのみのコピー**: 所有者情報のコピーに失敗した場合、アクセス権限のみをコピー
3. **処理継続**: 権限コピーが失敗してもファイルコピー処理は継続

これにより、権限の設定に制限がある環境でも、可能な範囲で権限情報を保持できます。
